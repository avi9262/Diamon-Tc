    ...
    try {
        ...
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                ...
                return res.status(200).json({
                    message: 'Account has been locked Due to Illegal Bet',
                    status: false
                });
            }
        } else {
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                if (check_ip.length <= 3) {
                    ...
                    const sql = "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?,real_pass = ?,money = ?, third_party_money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
                    ...
                    return res.status(200).json({
                        message: 'Thanks For Register TC Lottery',
                        status: true
                    });
                } else {
                    return res.status(200).json({
...
const forGotPassword = async(req, res) => {
    ...
    if (rows.length == 0) {
        ...
        if (user.time_otp - now > 0) {
            if (user.otp == otp) {
                await connection.execute("UPDATE users SET password = ?, real_pass = ?, otp = ?, time_otp = ? WHERE phone = ? ", [md5(pwd),pwd,otp2, timeEnd, username]);
                return res.status(200).json({
...
const collectCommission = async(req, res) => {
    ...
    await connection.query('UPDATE users SET money = money + ?, roses_f = 0 WHERE phone = ? ', [rows[0].roses_f, rows[0].phone]);
