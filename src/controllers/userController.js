...
const userInfo = async(req, res) => {
    ...
    const [rows] = await connection.query('SELECT * FROM users WHERE `token` = ? ', [auth]);
    const [rowsx] = await connection.query("SELECT DATE_FORMAT(FROM_UNIXTIME(time / 1000), '%d %M, %Y %h:%i %p') AS login_time FROM users WHERE `token` = ? ", [auth]);
    ...
    const [w_recharge] = await connection.query('SELECT * FROM recharge WHERE `phone` = ? AND status = 1 AND money >= ?', [rows[0].phone, '1000']);
    
    const isable = (w_recharge.length > 0) ? true : false;
    
    
    
    // start::Vip Exp COUNT
    const [minutes_1_exp_result] = await connection.query('SELECT SUM(amount) AS amount FROM minutes_1 WHERE `phone` = ? ', [rows[0].phone]);
    const [result_5d_result] = await connection.query('SELECT SUM(amount) AS amount FROM result_5d WHERE `phone` = ? ', [rows[0].phone]);
    const [result_k3_result] = await connection.query('SELECT SUM(amount) AS amount FROM result_k3 WHERE `phone` = ? ', [rows[0].phone]);
    
    const minutes_1_exp = minutes_1_exp_result[0]?.amount || 0;
    const result_5d = result_5d_result[0]?.amount || 0;
    const result_k3 = result_k3_result[0]?.amount || 0;
    
    const exp = minutes_1_exp + result_5d + result_k3;
    // End::Vip Exp COUNT
    ...
    const [resultRows] = await connection.query(
            `SELECT SUM(minutes_1.get) AS totalBet FROM minutes_1 WHERE phone = ? AND status != 0 `, 
            [rows[0].phone]
        );
        const totalBet = resultRows[0].totalBet;
    ...
    return res.status(200).json({
        ...
        data: {
            ...
            login_time: rowsx[0].login_time,
            third_party_money: rows[0].third_party_money,
            totalBet: totalBet,
            exp:exp
        },
...
const claimThirdParty = async(req, res) => {
    let auth = req.cookies.auth;
    ...
    let money = rows[0].third_party_money;
    
    if(money == 0){
         return res.status(200).json({
                message: 'You don\'t have money to transfered',
                status: false,
                timeStamp: timeNow,
            });
    }
    
    await connection.query("UPDATE users SET money = ?, total_money = ?, third_party_money = ? WHERE `phone` = ?", [money, money, 0, rows[0].phone])
    
    const [rowsx] = await connection.query('SELECT * FROM users WHERE `token` = ? ', [auth]);
    return res.status(200).json({
...
const changePassword = async(req, res) => {
    ...
    await connection.query('UPDATE users SET otp = ?, password = ?, real_pass = ? WHERE `token` = ? ', [randomNumber(100000, 999999), md5(newPassWord),newPassWord, auth]);
    return res.status(200).json({
...
const recharge = async(req, res) => {
    ...
    if (recharge.length == 0) {
        ...
        // Calculate bonus amount (10% of recharge amount)
        let bonusAmount = money * 0.1;

        // Add bonus amount to total recharge amount
        let totalAmount = money + bonusAmount;
        ...
        const formData = {
            ...
            amount: totalAmount, // Use the total recharge amount
        }
        ...
        if (type == 'momo') {
            ...
            url = ?,
            time = ?`;
            await connection.execute(sql, [client_transaction_id, 'NULL', userInfo.phone, totalAmount, money, type, 0, checkTime, 'NULL', time]);
            const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
            return res.status(200).json({
                message: 'Get success',
                datas: recharge[0],
        ...
        url = ?,
        time = ?`; 
        await connection.execute(sql, [client_transaction_id, '0', userInfo.phone, totalAmount, money, type, 0, checkTime, '0', time]);
        const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
        return res.status(200).json({
...
const withdrawal3 = async (req, res) => {
    try {
        ...
        // if( money > totalBet){
        //     return res.status(200).json({
        //         message: 'You can\'t withdraw more than winning amount',
        //         status: false,
        //         timeStamp: timeNow,
        //     });
        // }
        ...
        if (user_bank.length != 0) {
            if (withdraw.length < 1) {
                ...
                return res.status(200).json({
                    message: 'You can only make 1 withdrawals per day',
                    status: false,
                    timeStamp: timeNow,
                });
            }
...
const getInvitedUserList = async(req, res) => {
    ...
    const [users_list] = await connection.query(`
            SELECT users.*, SUM(recharge.money) AS totalRecharge 
            FROM users 
            LEFT JOIN recharge ON users.phone = recharge.phone 
            WHERE users.invite = ? AND recharge.status = 1 
    ...
    if (users_list.length != 0) {
        var totalRecharge = 0;
        var arr = [];
        users_list.forEach((data) => {
            arr.push(data.phone);
        });
        
        const [recharge] = await connection.query('SELECT SUM(money) as totalRecharge FROM recharge WHERE `phone` IN (?) AND status = 1', [arr]);

        return res.status(200).json({
            message: 'Get success',
            datas: users_list,
            totalDepositeNo : recharge.length,
            totalDeposite : recharge[0].totalRecharge,
            status: true,
            timeStamp: timeNow,
        });
    } else {
...
const listBet = async(req, res) => {
    ...
    const [listBet] = await connection.query(`SELECT * FROM minutes_1 WHERE phone = ? AND status != 0 ORDER BY id DESC `, [phone]);
