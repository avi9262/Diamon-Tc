...
import jiliGamesController, { GAME_CATEGORIES_MAP } from "../controllers/jiliGamesController"
...
const initWebRouter = (app) => {
   ...
   router.get("/jili/lobby", middlewareController, jiliGamesController.gameCategoriesPage(GAME_CATEGORIES_MAP.LOBBY));
   ...
   router.get("/jili/poker", middlewareController, jiliGamesController.gameCategoriesPage(GAME_CATEGORIES_MAP.POKER));
    //Pages
    router.get('/arkpay', middlewareController, ekPayController.arkpayHandler);
    router.get('/sunpayb', middlewareController, ekPayController.sunpaybHandler);
    router.post('/webhook', webhookController.handleWebhook);
    router.get('/EKPAY', middlewareController, ekPayController.ekPayHandler);
    router.get('/usdt', middlewareController, ekPayController.usdtHandler);
    router.get('/usdtout', middlewareController, ekPayController.usdtoutHandler);
    router.get('/ekqr', middlewareController, ekPayController.ekqrHandler);
    router.get('/TB-PAY', middlewareController, ekPayController.usdtpayHandler);
    router.post('/submit-form', formController.submitForm);
    router.post('/save-utr', utrController.saveUTR); //UTR Save
    router.get('/confirm-payment', confirmPaymentController.confirmPayment); // Add this route for confirming payment
    router.get('/confirm-paymentusdt', confirmPaymentController.confirmPaymentUsdt);
    router.post('/create-order', paymentController.createOrder);
    // Define route for handling payment notifications
    router.post('/sunpaynotify', paymentNotificationController.handlePaymentNotification);
    
    // page account
    router.get('/keFuMenu', accountController.keFuMenu);
    router.get('/privacy', accountController.privacyPage);
    ...
    router.post('/api/promotion/collectCommision', accountController.collectCommission);

    // page home
    router.get('/', (req, res) => {
        return res.redirect('/home/splash');
    });
    router.get('/home/splash', homeController.homeSplash);
    ...
    router.get('/wallet/sunpay', middlewareController, homeController.sunpayPage);
    router.get('/wallet/ekpayment', middlewareController, homeController.ekpaymentPage);
    router.get('/wallet/twopayment', middlewareController, homeController.twopaymentPage);
    router.get('/wallet/withdrawal', middlewareController, homeController.withdrawalPage);
    router.get('/wallet/withusdt', middlewareController, homeController.withusdtPage);
    ...
    router.get('/safe', middlewareController, homeController.safePage);
    router.get('/language', middlewareController, homeController.languagePage);
    ...
    router.get('/gamehistory', middlewareController, homeController.gamehistoryPage);
    router.get('/teamdata', middlewareController, homeController.teamdataPage);
    router.get('/transactionhistory', middlewareController, homeController.transactionhistoryPage);
    router.get('/commissiondetails', middlewareController, homeController.commissiondetailsPage);
    ...
    router.post('/api/webapi/claim/third_party',middlewareController, userController.claimThirdParty);
    ...
    router.post('/api/webapi/search', middlewareController, userController.search); // register
     router.get('/api/webapi/getInvitedUserList', middlewareController, userController.getInvitedUserList);

    router.post('/api/webapi/bet', middlewareController, userController.listBet);
